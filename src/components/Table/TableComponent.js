...
const TableComponent = ({ items }) => {
  ...
  const fieldsToDisplay = [
    ...
    { key: 'Bay', value: items.custom_fields && items.custom_fields["Bay #"] ? items.custom_fields["Bay #"].value : 'N/A' },
  ...
  return (
    <FlatList
      ...
      renderItem={({ item, index }) => (
        <View style={[styles.row, { backgroundColor: index % 2 === 0 ? '#fff' : "rgba(161, 161, 161, 0.28)" }]}>
          <View style={styles.column}>
            <Text style={styles.key}>{item.key}</Text>
          </View>
          <View style={styles.column}>
            <Text numberOfLines={1} style={[styles.value, { flexWrap: 'wrap' }]}>
              {item.value || 'N/A'}
            </Text>
          </View>
        </View>
